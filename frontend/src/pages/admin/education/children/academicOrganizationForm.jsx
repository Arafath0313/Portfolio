...
import { academicOrganizationSchema } from "../../../../schemas";
...
const AcademicOrganizationForm = () => {
  ...
  const {
    ...
    resolver: zodResolver(academicOrganizationSchema),
    defaultValues: {
  ...
  useEffect(() => {
    if (isEditMode) {
      academicOrganizationService.getById(childId).then((data) => {
        reset(data);
      }).catch(() => {
        toast.error("Failed to fetch details.");
        navigate(`/admin/education/${educationId}/organizations`);
      });
    }
  ...
  const onSubmit = async (data) => {
    try {
      if (isEditMode) {
        await academicOrganizationService.update(childId, data);
        toast.success("Updated successfully.");
      } else {
        await academicOrganizationService.create(data);
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Link
            to={`/admin/education/${educationId}/organizations`}
      ...
      <Card className="p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <Input label="Organization Name *" {...register("organizationName")} error={errors.organizationName?.message} />
     ...
     <Input label="Logo URL" {...register("logo")} error={errors.logo?.message} />
     <div className="sm:col-span-2">
        <Textarea label="Description" rows={4} {...register("description")} error={errors.description?.message} />
          ...
          <div className="flex justify-end gap-3 border-t border-slate-100 pt-6 dark:border-slate-700">
            <Link to={`/admin/education/${educationId}/organizations`}>
